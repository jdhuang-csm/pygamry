...
if __name__ == '__main__':
    ...
    for n in range(args.max_repeats):
        ...
        if seq.meas_v_min <= args.stop_v_min:
            print('STOPPING CHARGE/DISCHARGE: measured voltage {:.3f} V is below low threshold ({:.3f} V)'.format(
                seq.meas_v_min, args.stop_v_min)
            )
            stop = True

        if seq.meas_v_max >= args.stop_v_max:
            print('STOPPING CHARGE/DISCHARGE: measured voltage {:.3f} V is above high threshold ({:.3f} V)'.format(
        ...
        if stop:
            ...
            print('Resting for {:.1f} s between cycles...'.format(args.hybrid_rest_time))
    ...
    if args.voltage_finish:
        ...
        print('Voltage finish current limits: ({:.4f} A, {:.4f} A)'.format(i_min, i_max))
