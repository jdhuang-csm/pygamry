...
parser.add_argument('--equil_time', type=float, default=3600)

parser.add_argument('--charge_idc', type=float, default=0.1)
parser.add_argument('--charge_duration', type=float, default=3600)
parser.add_argument('--charge_max_repeats', type=float, default=90)

parser.add_argument('--charge_idc', type=float, default=0.1)
parser.add_argument('--charge_duration', type=float, default=3600)
parser.add_argument('--charge_max_repeats', type=float, default=90)

parser.add_argument('--charge_voltage_finish', default=False, action='store_true')
parser.add_argument('--charge_finish_v', type=float, default=0.0)
parser.add_argument('--charge_finish_i_thresh', type=float, default=0.001)
parser.add_argument('--charge_finish_t_sample', type=float, default=1)
parser.add_argument('--charge_finish_duration', type=float, default=3600)
parser.add_argument('--charge_finish_rest_time', type=float, default=60)
...
if __name__ == '__main__':
    ...
    for n in range(args.repeats):
        ...
        if args.voltage_finish and seq.meas_v_end * i_sign >= args.finish_v * i_sign:
            print('STOPPING CHARGE/DISCHARGE: '
    ...
    if args.voltage_finish:
        print('Resting for {:.1f} s before voltage finish...'.format(args.finish_rest_time))
        time.sleep(args.finish_rest_time)

        dt_pstatic = DtaqPstatic()

        pstatic_file = os.path.join(args.data_path, f'PSTATIC-FINISH_{args.file_suffix}.DTA')
        ...
        if i_sign > 0:
            ...
            i_max = args.hybrid_i_init * 1.05
        else:
            i_min = args.hybrid_i_init * 1.05
            i_max = abs(args.finish_i_thresh) * i_sign

        dt_pstatic.run(pstat, args.finish_v, args.finish_duration, args.finish_t_sample,
                       i_min=i_min, i_max=i_max,
                       result_file=pstatic_file, kst_file=kst_file)
